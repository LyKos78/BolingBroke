...
module.exports = {
    ...
    run: async (client, message, args, prefix, color) => {
        ...
        const list = message.guild.members.cache.filter(m => m.user.bot && m.permissions.has(PermissionFlagsBits.Administrator));

        if (list.size === 0) return message.channel.send("Aucun bot administrateur trouvé.");
